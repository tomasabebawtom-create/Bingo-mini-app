...
def withdraw_command(message):
    bot.reply_to(message, "💸 የሚወጣውን መጠን ላክ (ለምሳሌ: 100)")
...
def process_withdraw_amount(message):
    try:
        ...
        if response.status_code == 200:
            ...
            bot.reply_to(message, "❌ በቂ ቀሪ ሂሳብ የለዎትም።")
        else:
            bot.reply_to(message, "❌ ጥያቄውን መፍጠር አልተቻለም።")
    except ValueError:
        bot.reply_to(message, "❌ ትክክለኛ ቁጥር ብቻ ይላኩ።")
...
def process_deposit_amount(message):
    """
    ገንዘብ አይጨመርም! "pending" ትዕዛዝ ብቻ ይፈጠራል።
    ገንዘቡ የሚጨመረው አድሚኑ Approve ሲጫን ብቻ ነው።
    """
    try:
        amount = float(message.text)
        if amount <= 0:
        ...
        if response.status_code != 200:
            bot.reply_to(
                message,
                "❌ ጥያቄ መፍጠር አልተቻለም። እባክዎ እንደገና ይሞክሩ።",
            )
            return
        ...
        bot.reply_to(
            ...
            f"📱 {TELEBIRR_NUMBER}\n\n"
            "ከላኩ በኋላ አድሚን አረጋግጦ ገንዘብዎን ይጨምርልዎታል።",
        )
        ...
        bot.reply_to(
            message,
            "❌ ከሰርቨሩ ጋር መገናኘት አልተቻለም። እባክዎ ቆይተው ይሞክሩ።",
        )
